...
WEBAPP_URL = os.getenv("WEBAPP_URL", "https://kaionavan.github.io/Tgbot1/webapp.html")
...
# =============================================
#  САМООБУЧЕНИЕ (DEEP STUDY)
# =============================================
async def deep_study(app: Application, chat_id: int, topic: str, data: dict) -> None:
    """
    Глубокое изучение темы:
    1. Несколько поисковых запросов
    2. AI синтезирует всё в структурированный отчёт
    3. Сохраняет тему в память
    4. Отправляет отчёт файлом
    """
    try:
        await app.bot.send_message(chat_id, 
            f"🧠 *Начинаю глубокое изучение:* _{topic}_\n\n"
            f"📡 Провожу поиск по нескольким запросам...",
            parse_mode="Markdown"
        )

        # Шаг 1: Генерируем несколько поисковых запросов по теме
        queries_prompt = (
            f"Тема для изучения: {topic}\n\n"
            f"Сгенерируй 5 конкретных поисковых запросов чтобы изучить эту тему со всех сторон. "
            f"Выдай ТОЛЬКО список запросов, каждый на новой строке, без нумерации и лишних слов."
        )
        queries_raw, _ = await get_best_ai_response(queries_prompt, "Ты помощник для поиска информации.")
        
        if not queries_raw:
            queries = [topic, f"{topic} подробно", f"как работает {topic}"]
        else:
            queries = [q.strip() for q in queries_raw.strip().split('\n') if q.strip()][:5]

        # Шаг 2: Выполняем поиск по каждому запросу
        all_results = []
        for i, query in enumerate(queries, 1):
            await app.bot.send_message(chat_id,
                f"🔍 Поиск {i}/{len(queries)}: _{query}_",
                parse_mode="Markdown"
            )
        ...
        if not all_results:
            await app.bot.send_message(chat_id, "❌ Не удалось найти информацию по теме.")
            return

        combined_search = "\n\n".join(all_results[:4])

        # Шаг 3: AI синтезирует полный отчёт
        await app.bot.send_message(chat_id, "📝 Синтезирую отчёт из найденной информации...")

        report_prompt = (
            f"Ты эксперт-аналитик. На основе результатов поиска напиши ПОЛНЫЙ структурированный отчёт по теме: {topic}\n\n"
            f"Результаты поиска:\n{combined_search[:6000]}\n\n"
            f"СТРУКТУРА ОТЧЁТА:\n"
            f"1. 📌 Краткое резюме (3-5 предложений)\n"
            f"2. 📚 Что это такое (определение и суть)\n"
            f"3. 🔧 Как это работает / Основные принципы\n"
            f"4. ✅ Плюсы и возможности\n"
            f"5. ⚠️ Минусы и риски\n"
            f"6. 💼 Применение на практике\n"
            f"7. 🚀 Как начать / С чего начать изучение\n"
            f"8. 📖 Ключевые термины\n\n"
            f"Пиши подробно, на русском языке. Отвечай только содержанием отчёта."
        )

        report, ai_name = await get_best_ai_response(report_prompt, "Ты эксперт-аналитик. Пиши подробные отчёты.")

        if not report:
            await app.bot.send_message(chat_id, "❌ Не удалось синтезировать отчёт.")
            return

        # Шаг 4: Сохраняем тему в память
        if topic not in data.get("topics", []):
            data.setdefault("topics", []).append(topic)
        
        # Добавляем краткое резюме в историю чата
        add_message(data, "Агент", f"Изучена тема: {topic}. " + report[:200])
        save_data(data)

        # Шаг 5: Формируем файл отчёта
        timestamp = datetime.now().strftime("%d%m%Y_%H%M%S")
        filename = f"study_{timestamp}.txt"
        
        file_content = (
            f"ОТЧЁТ ОБ ИЗУЧЕНИИ ТЕМЫ\n"
            f"{'='*50}\n"
            f"Тема: {topic}\n"
        ...
        with open(filename, "rb") as f:
            await app.bot.send_document(
                chat_id=chat_id,
                document=f,
                filename=f"Изучение_{topic[:30]}.txt",
                caption=(
                    f"✅ *Тема изучена: {topic}*\n\n"
                    f"📊 Отчёт: {len(report.splitlines())} строк\n"
                    f"🤖 {ai_name}\n\n"
                    f"💾 Тема сохранена в памяти!"
                ),
                parse_mode="Markdown"
            )
        ...
        logger.error(f"deep_study error: {e}")
        await app.bot.send_message(chat_id, f"❌ Ошибка при изучении темы: {e}")


def is_study_request(text: str) -> tuple[bool, str]:
    """Определяет, просит ли пользователь изучить тему. Возвращает (да/нет, тема)"""
    patterns = [
        r'изучи\s+(.+)',
        r'изучить\s+(.+)',
        r'глубоко\s+изучи\s+(.+)',
        r'подробно\s+изучи\s+(.+)',
        r'исследуй\s+(.+)',
    ]
    for pattern in patterns:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработчик команды /start — ВСЕГДА отвечает, три уровня fallback"""
    # Уровень 1: Полный ответ со статистикой
    try:
        data = load_data()
        if "topics" not in data:
            data["topics"] = []
        if "chats" not in data:
            data["chats"] = {"main": {"name": "Основной", "history": []}}
        if "current" not in data or data["current"] not in data["chats"]:
            data["current"] = "main"
        chat = data["chats"].get(data["current"], {"name": "Основной", "history": []})
        ...
        logger.error(f"start_command level1 error: {e}")
    
    # Уровень 2: Упрощённый ответ без Markdown
    try:
        await update.message.reply_text(
            "👑 Приветствую, господин! Я ваш AI-агент, готов служить.\n\nЧем могу помочь?",
            reply_markup=main_keyboard()
        )
        return
    except Exception as e:
        logger.error(f"start_command level2 error: {e}")
    
    # Уровень 3: Минимальный ответ без клавиатуры
    try:
        await update.message.reply_text("👑 Готов служить, господин!")
    except Exception as e:
        logger.error(f"start_command level3 error: {e}")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    is_study, study_topic = is_study_request(text)
    if is_study and study_topic:
        data = load_data()
        asyncio.create_task(deep_study(context.application, chat_id, study_topic, data))
        return
