...
def main_keyboard() -> ReplyKeyboardMarkup:
    ...
    webapp_url = (WEBAPP_URL + "#groq=" + GROQ_KEY) if GROQ_KEY else WEBAPP_URL
    keyboard = [
...
# =============================================
async def deep_study(app, chat_id: int, topic: str, data: dict) -> None:
    """Глубокое изучение темы с поиском и отчётом"""
    try:
        await app.bot.send_message(chat_id,
            f"🧠 *Начинаю изучение:* _{topic}_\n\n📡 Ищу информацию...",
            parse_mode="Markdown"
        )
        # Генерируем поисковые запросы
        queries_raw, _ = await get_best_ai_response(
            f"Тема: {topic}\nДай 4 поисковых запроса чтобы изучить тему со всех сторон. Только список, каждый с новой строки.",
            "Ты помощник для поиска. Отвечай только списком запросов."
        )
        queries = [q.strip() for q in (queries_raw or topic).split("\n") if q.strip()][:4]
        if not queries:
            queries = [topic, f"{topic} как работает", f"{topic} применение"]

        # Ищем по каждому запросу
        all_results = []
        for i, query in enumerate(queries, 1):
            await app.bot.send_message(chat_id, f"🔍 Поиск {i}/{len(queries)}: _{query}_", parse_mode="Markdown")
        ...
        if not all_results:
            await app.bot.send_message(chat_id, "❌ Не удалось найти информацию.")
            return

        combined = "\n\n".join(all_results[:3])
        await app.bot.send_message(chat_id, "📝 Синтезирую отчёт...")

        report, ai_name = await get_best_ai_response(
            f"Тема: {topic}\n\nДанные поиска:\n{combined[:5000]}\n\n"
            f"Напиши ПОЛНЫЙ структурированный отчёт:\n"
            f"1. 📌 Краткое резюме\n2. 📚 Что это такое\n3. 🔧 Как работает\n"
            f"4. ✅ Плюсы\n5. ⚠️ Минусы\n6. 💼 Применение\n7. 🚀 Как начать\n"
            f"Подробно, на русском языке.",
            "Ты эксперт-аналитик. Пиши подробные отчёты на русском."
        )
        ...
        add_message(data, "Агент", f"Изучена тема: {topic}")
        save_data(data)

        # Файл отчёта
        ts = datetime.now().strftime("%d%m%Y_%H%M%S")
        fname = f"study_{ts}.txt"
        file_content = (
            f"ОТЧЁТ: {topic}\n{'='*50}\n"
        ...
        with open(fname, "rb") as f:
            await app.bot.send_document(
                ...
                filename=f"Изучение_{topic[:25]}.txt",
                caption=f"✅ *Тема изучена: {topic}*\n🤖 {ai_name}\n💾 Сохранено в памяти!",
                parse_mode="Markdown"
            )
        os.remove(fname)

        short = report[:800] + ("..." if len(report) > 800 else "")
        await app.bot.send_message(chat_id, f"📌 *Краткое резюме:*\n\n{short}", parse_mode="Markdown")

    except Exception as e:
        logger.error(f"deep_study error: {e}")
        await app.bot.send_message(chat_id, f"❌ Ошибка при изучении: {e}")


def is_study_request(text: str):
    """Проверяет запрос на самообучение"""
    import re
    for pattern in [r'изучи\s+(.+)', r'изучить\s+(.+)', r'исследуй\s+(.+)', r'глубоко\s+изучи\s+(.+)']:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработчик команды /start — ВСЕГДА отвечает (3 уровня защиты)"""
