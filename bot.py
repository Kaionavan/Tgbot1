...
async def analyze_image(image_bytes: bytes, prompt: str = "") -> Optional[str]:
    ...
    question = prompt or "Опиши подробно что видишь на изображении. Если есть текст — прочитай его весь. Отвечай на русском."
    ...
    try:
        async with httpx.AsyncClient(timeout=90) as client:
            ...
            data = r.json()
            logger.info(f"Gemini vision response status: {r.status_code}")
            
            if "candidates" in data:
                result = data["candidates"][0]["content"]["parts"][0]["text"]
                logger.info(f"Gemini vision OK: {result[:50]}")
                return result
            elif "error" in data:
                logger.error(f"Gemini vision API error: {data['error']}")
                return f"Gemini ошибка: {data['error'].get('message', 'неизвестно')}"
            else:
                logger.error(f"Gemini vision unexpected response: {data}")
                return None
    except Exception as e:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        data = load_data()
        # Защита от старого формата данных
        if "topics" not in data:
            data["topics"] = []
        if "last_code" not in data:
            data["last_code"] = ""
        current_chat = data["chats"].get(data["current"], list(data["chats"].values())[0])
        ...
        await update.message.reply_text(
            ...
            "💻 Код • 🎤 Голос • 📸 Фото • 🌐 Поиск • ⏰ Напоминания\n\n"
            "👇 Используй кнопки!",
...
async def restart_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            "🔄 *Бот перезапущен!* Готов к работе 👇",
            parse_mode="Markdown",
            reply_markup=main_keyboard()
        )
    except Exception as e:
        logger.error(f"restart_command error: {e}")
