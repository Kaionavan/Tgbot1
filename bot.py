...
async def ask_openrouter(prompt: str, system: str) -> tuple[str | None, str]:
    ...
    for model in OPENROUTER_MODELS:
        try:
            async with httpx.AsyncClient(timeout=40) as client:
                r = await client.post(
                    ...
                    headers={
                        ...
                        "HTTP-Referer": "https://t.me/agentbot",
                        "X-Title": "AgentBot"
                    },
                    json={
                        "model": model,
            ...
            err = data.get("error", {})
            logger.warning(f"OpenRouter {model}: {err.get('message','')[:80]}")
        except Exception as e:
    ...
    try:
        ...
        if "message" in data:
            ...
            if content and content[0].get("text"):
                return content[0]["text"], "Cohere/command-r"
        logger.warning(f"Cohere: {data.get('message','unknown')}")
    except Exception as e:
        logger.warning(f"Cohere: {e}")
    return None, ""

async def ask_gemini_text(prompt: str, system: str) -> tuple[str | None, str]:
...
async def ask_ai(prompt: str, system: str = "") -> tuple[str | None, str]:
    """
    Умная очередь: Groq → OpenRouter → Cohere → Gemini
    Каждый следующий — резерв если предыдущий не сработал
    """
    if not system:
