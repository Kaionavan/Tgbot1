...
def is_code_task(text):
    keywords = ["создай", "напиши", "сделай", "код", "приложение", "скрипт",
                "программу", "бот", "сайт", "функцию", "calculator", "калькулятор", "игру"]
        ...
        if is_code_task(text):
            ...
            if code:
                ...
                with open(filename, 'rb') as f:
                    await app.bot.send_document(chat_id, document=f, filename=filename, caption=f"📁 `{filename}`")
                os.remove(filename)
                return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Используй кнопки внизу или пиши что нужно!",
        reply_markup=main_keyboard()
    )

async def new_chat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Введи название нового чата или нажми /skip для автоматического названия:"
    )
...
async def clear_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_data(data)
    await update.message.reply_text("🗑 История текущего чата очищена!", reply_markup=main_keyboard())

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "❓ Что я умею:\n\n"
        "💬 *Новый чат* — создать новый чат\n"
        "📂 *Мои чаты* — список и переключение чатов\n"
        "🧠 *Память* — что я помню о тебе\n"
        "🗑 *Очистить чат* — удалить историю\n\n"
        "Просто напиши:\n"
        "• *Объясни [тему]* — изучу и расскажу\n"
        "• *Напиши код [задача] на [язык]* — пришлю файл\n"
        "• *Создай приложение [описание]* — напишу и пришлю",
        parse_mode="Markdown",
        reply_markup=main_keyboard()
    )
...
async def skip_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("waiting_chat_name"):
        ...
        data["chats"][key] = {"name": name, "history": [], "created": datetime.now().strftime("%d.%m.%Y %H:%M")}
        data["current_chat"] = key
        save_data(data)
        await update.message.reply_text(f"✅ Новый чат создан: **{name}**", reply_markup=main_keyboard())
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("switch_"):
        ...
        if key in data["chats"]:
            data["current_chat"] = key
            save_data(data)
            await query.edit_message_text(f"✅ Переключился на чат: **{data['chats'][key]['name']}**")

    elif query.data == "show_delete":
        keyboard = []
        for key, chat in data["chats"].items():
            if key != "default":
                keyboard.append([InlineKeyboardButton(f"🗑 {chat['name']}", callback_data=f"delete_{key}")])
        if not keyboard:
            await query.edit_message_text("❌ Нет чатов для удаления (основной чат нельзя удалить)")
        else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Кнопки меню
    if text == "💬 Новый чат":
        await new_chat_cmd(update, context)
        return
    if text == "📂 Мои чаты":
        await list_chats_cmd(update, context)
        return
    if text == "🧠 Память":
    ...
    if context.user_data.get("waiting_chat_name"):
        ...
        data = load_data()
        key = f"chat_{int(datetime.now().timestamp())}"
        data["chats"][key] = {"name": text, "history": [], "created": datetime.now().strftime("%d.%m.%Y %H:%M")}
        data["current_chat"] = key
        save_data(data)
        await update.message.reply_text(f"✅ Новый чат создан: **{text}**", reply_markup=main_keyboard())
        return
