...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if waiting_for == "new_chat_name":
        ...
        await msg.delete()
        await send_long_message(chat_id, result, context.application, parse_mode="Markdown")
        return
...
async def get_weather(city: str = "Moscow") -> str:
    ...
    try:
        url = f"https://api.openweathermap.org/data/2.5/forecast?q={city}&appid={WEATHER_KEY}&units=metric&lang=ru&cnt=8"
...
async def get_youtube_transcript(video_id: str) -> str:
    """Получает субтитры YouTube видео через API"""
    try:
        # Получаем информацию о видео
        url = f"https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id={video_id}&key={YOUTUBE_KEY}"
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(url)
        d = r.json()
        if not d.get("items"):
        ...
        desc = item["snippet"]["description"][:2000]
    ...
    try:
        if fname_lower.endswith(".txt") or fname_lower.endswith(".md"):
            ...
            try:
                ...
                return "❌ PDF анализ недоступен. Отправь файл как .txt"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    allowed = [".txt", ".pdf", ".docx", ".md", ".csv"]
    fname = doc.file_name or "document"
    ext = "." + fname.rsplit(".", 1)[-1].lower() if "." in fname else ""

    if ext not in allowed:
        await update.message.reply_text(
            f"❌ Формат {ext} не поддерживается.\nПоддерживаю: {', '.join(allowed)}"
        )
        return
    ...
    try:
        ...
        await msg.delete()
        await send_long_message(update.effective_chat.id, result, context.application, parse_mode="Markdown")
